...
export class AdicionaCreditoUsuarioUseCase {
  ...
  async execute({ idAdmin, email, quantidade }: IRequest): Promise<{msg: string, usuario: IUsuarioDTO}> {
    ...
    return { msg: 'Crédito adicionado com sucesso!', usuario: { ...usuario, avatar_url: usuario.avatarUrl } };
  }
}
